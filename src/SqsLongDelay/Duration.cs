...
namespace SqsLongDelay
{
    ...
    public static class Duration
    {
        ...
        public static TimeSpan ToTimeSpan(string duration)
        {
            ...
            {
                double durationValue = double.Parse(group.Value);
                TimeSpan durationSpan;

                switch(group.Name)
                {
                    case "Days":
                        durationSpan = TimeSpan.FromDays(durationValue);
                        break;
                    case "Hours":
                        durationSpan = TimeSpan.FromHours(durationValue);
                        break;
                    case "Minutes":
                        durationSpan = TimeSpan.FromMinutes(durationValue);
                        break;
                    case "Seconds":
                    case "Fractions":
                        durationSpan = TimeSpan.FromSeconds(durationValue);
                        break;
                    default:
